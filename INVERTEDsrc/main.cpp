...
// Text color palettes — pre-inverted so hardware inversion shows the correct color
// invertDisplay(true) flips every pixel, so we draw the bitwise inverse of what we want shown.
static const uint16_t TEXT_COLORS[] = {
  0xFFFF,  // 0 "white"  → draws white  → appears black  (most readable on white bg)
  0xF81F,  // 1 "green"  → draws magenta → appears green
  0xF800,  // 2 "cyan"   → draws red     → appears cyan
  0x001F,  // 3 "yellow" → draws blue    → appears yellow
  0x02DF,  // 4 "orange" → draws inv.    → appears orange
  0x07FF,  // 5 "red"    → draws cyan    → appears red
};
static const uint16_t MULTI_COLORS[] = {
  0xF800,  // appears cyan
  0xF81F,  // appears green
  0x001F,  // appears yellow
  0x07FF,  // appears red
  0x07E0,  // appears magenta
  0x02DF,  // appears orange
  0xFFFF,  // appears black
};
...
void renderPage() {
  ...
  while (lineStart <= bodyLen) {
    ...
    while (line.length() > 0) {
      ...
      int colorIdx = constrain(wc_text_color_idx, 0, 6);
      if (colorIdx == 6) {
        gfx->setTextColor(MULTI_COLORS[colorStep % MULTI_COLOR_COUNT]);
      } else {
        gfx->setTextColor(TEXT_COLORS[colorIdx]);
      }
...
// (stripMarkdown defined below, forward declared here)
static String stripMarkdown(const String &src);

bool fetchAndRender() {
  if (strlen(wc_readme_repo) == 0) {
    showStatus("No repo set - hold BOOT to configure");
    return false;
  }
  String fetchUrl = "https://raw.githubusercontent.com/";
  fetchUrl += wc_readme_repo;
  fetchUrl += "/HEAD/README.md";

  wc_body = "";
  wc_body = https_get_string(fetchUrl);
  if (wc_body.isEmpty()) return false;

  wc_body = stripMarkdown(wc_body);
  wc_page_start = 0;
  renderPage();
  return true;
}

// Strip Markdown syntax for plain-text display on the CYD.
// Handles: headers, bold/italic, links, images, code fences, HTML tags, blockquotes.
static String stripMarkdown(const String &src) {
  String out;
  ...
  while (i < n) {
    ...
    // HTML tags <...> — strip tag, keep nothing
    if (c == '<') {
      while (i < n && src[i] != '>') i++;
      i++;
      continue;
    }
    // Images ![alt](url) — skip entire construct
    if (c == '!' && i+1 < n && src[i+1] == '[') {
    ...
    // Code fences ``` — skip the fence marker line
    ...
    // Bold/italic ** * __ _ — strip markers
    if ((c == '*' || c == '_') && i+1 < n && (src[i+1] == '*' || src[i+1] == '_')) {
      i += 2; continue;
    }
    if (c == '*' || c == '_') {
      i++; continue;
    }
    // Blockquote > at line start — strip the >
    if (c == '>' && (i == 0 || src[i-1] == '\n')) {
      i++;
      while (i < n && src[i] == ' ') i++;
      continue;
...
// Show the active repo in the status bar
void showActiveSource() {
  char msg[64];
  snprintf(msg, sizeof(msg), "README: %s", wc_readme_repo);
  showStatus(msg);
}
...
void setup() {
  ...
  gfx->invertDisplay(true);  // white-background display fix
