...
static char wc_readme_repo[128] = "";  // username/reponame
...
static void wcLoadSettings() {
  ...
  String readmerepo = prefs.getString("readmerepo", "");
  ...
  readmerepo.toCharArray(wc_readme_repo, sizeof(wc_readme_repo));
  wc_has_settings = (ssid.length() > 0);
}

static void wcSaveSettings(const char *ssid, const char *pass,
                           const char *readmeRepo, int colorIdx, int textSize) {
  Preferences prefs;
  prefs.begin("githubraw", false);
  prefs.putString("ssid",       ssid);
  prefs.putString("pass",       pass);
  prefs.putString("readmerepo", readmeRepo);
  ...
  strncpy(wc_readme_repo, readmeRepo, sizeof(wc_readme_repo) - 1);
...
static void wcHandleRoot() {
  ...
  html += "' placeholder='Leave blank if open network' maxlength='63'>"
    "<label>&#128196; GitHub Repo (username/reponame):</label>"
    "<input type='text' name='readmerepo' value='";
  html += String(wc_readme_repo);
  html += "' placeholder='e.g. torvalds/linux' maxlength='127' required>"
    "<p style='color:#445566;font-size:0.82em;margin:2px 0 10px;text-align:left;'>"
    "Fetches README.md from the repo — Markdown is stripped for plain-text display.</p>";
...
static void wcHandleSave() {
  ...
  wcSaveSettings(ssid.c_str(), pass.c_str(),
    portalServer->hasArg("readmerepo") ? portalServer->arg("readmerepo").c_str() : "",
